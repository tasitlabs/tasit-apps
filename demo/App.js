...
export default class App extends React.Component {
  state = {
	...
	componentDidMount() {
		...
		const address = "0x7357589f8e367c2C31F51242fB77B350A11830F3";
		...
		let wallet2mnemonic = "radar blur cabbage chef fix engine embark joy scheme fiction master release";
		...
		console.assert(wallet3.address !== undefined, "wallet.address from randonly created");
		...
		fundedWallet.sign(transaction).then((signedTransaction) => {
			...
			let signed = "0xf86c808504a817c8008252089488a5c2d9919e46f883eb62f7b8" +
		...
		wallet.signMessage("Hello World!").then((signature) => {
		    ...
		    let sign = "0xea09d6e94e52b48489bd66754c9c02a772f029d4a2f136bba9917a" +
		...
		wallet.signMessage(binaryData).then((signature) => {
				...
				let signed = "0x5e9b7a7bd77ac21372939d386342ae58081a33bf53479152c87c" +
		...
		fundedWallet.getBalance().then((balance) => {
				console.assert(utils.parseEther(balance).gt(utils.bigNumberify(0)), "funded wallet should have ethers");
		});

		mainnetProvider.resolveName("registrar.firefly.eth").then(function(address) {
		     console.assert(address === "0x6fC21092DA55B392b045eD78F4732bff3C580e2c", "provider.resolveName()");
		});

		mainnetProvider.lookupAddress("0x6fC21092DA55B392b045eD78F4732bff3C580e2c").then(function(name) {
		 	console.assert(name === "registrar.firefly.eth", "provider.lookupAddress()");
		});
		...
		contract.on("ValueChanged", (author, oldValue, newValue, event) => {
		    console.assert(author === fundedWallet.address, "ValueChanged event should have fundedWallet.address as author");
		    console.log(oldValue);
		    console.log(newValue);
		    console.log(event.blockNumber);
		});

		contract.setValue(newValue).then(tx => {
				console.log("ropsten contract write tx: ", tx.hash);
				return tx.wait();
		}).then(() => {
			contract.getValue().then(currentValue => {
					console.assert(currentValue === newValue, "contract value should changed");
			});
		}).catch((e) => {
			console.log(e);
		});
